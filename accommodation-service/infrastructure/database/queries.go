...
func GetAllAccommodations(DB *sql.DB, filter *utility.Filter) (*sql.Stmt, error) {
	query := `SELECT a.id, a.host_id, a.name, a.benefits, a.min_guests, a.max_guests, a.base_price, json_agg(DISTINCT r) as ratings `
	query += ` FROM Accommodation a`
	query += ` LEFT JOIN Rating r ON a.id = r.accommodation_id`
	query += FormFilterWhereClause(filter)
	query += ` GROUP BY a.id, a.host_id, a.name, a.benefits, a.min_guests, a.max_guests, a.base_price`
